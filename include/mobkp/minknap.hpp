...
#define MOBKP_MINKNAP_MAXSTATES 400000

#define MOBKP_MINKNAP_SYNC 5        /* when to switch to linear scan in bins */
#define MOBKP_MINKNAP_SORTSTACK 200 /* depth of stack used in qsort */
#define MOBKP_MINKNAP_MINMED 100    /* find exact median in qsort if larger size */

#define MOBKP_MINKNAP_TRUE 1
#define MOBKP_MINKNAP_FALSE 0

#define MOBKP_MINKNAP_LEFT 1
#define MOBKP_MINKNAP_RIGHT 2

#define MOBKP_MINKNAP_PARTIATE 1
#define MOBKP_MINKNAP_SORTALL 2

#define MOBKP_MINKNAP_MAXV (8 * sizeof(btype)) /* number of bits in a long integer */
#define MOBKP_MINKNAP_PMAX 1                   /* profit of worlds most efficient item  */
#define MOBKP_MINKNAP_WMAX 0                   /* weight of worlds most efficient item  */
...
#define MOBKP_MINKNAP_DIFF(a, b) ((int)((b) - (a) + 1))
#define MOBKP_MINKNAP_NO(a, p) ((int)((p) - (a)->fitem + 1))
#define MOBKP_MINKNAP_N(a, p) ((int)((p) - (a)->d.set1))
#define MOBKP_MINKNAP_L(x) ((long)(x))
#define MOBKP_MINKNAP_SZ(a) (*(((int *)(a)) - 4) - 1)
...
namespace mobkp::minknap {

typedef int boolean;
typedef std::int64_t ntype;  /* number of states/items   */
typedef std::int64_t itype;  /* item profits and weights */
typedef std::int64_t stype;  /* sum of pofit or weight   */
typedef std::int64_t ptype;  /* product type (sufficient precision) */
typedef std::uint64_t btype; /* binary representation of solution */
...
struct allinfo {        /* all problem information */
  ...
  item *vitem[MOBKP_MINKNAP_MAXV];  /* current last MAXV items */
  item *ovitem[MOBKP_MINKNAP_MAXV]; /* optimal set of items    */
...
state *findvect(stype ws, state *f, state *l) {
  ...
  if (f > l)
    throw("findvect: empty set");
  if (f->wsum > ws)
    return NULL;
  if (l->wsum <= ws)
    return l;

  while (l - f > MOBKP_MINKNAP_SYNC) {
    m = f + (l - f) / 2;
    if (m->wsum > ws) {
      l = m - 1;
    } else {
...
void push(allinfo *a, int side, item *f, item *l) {
  interval *pos = NULL;
  switch (side) {
    case MOBKP_MINKNAP_LEFT:
      pos = a->intv1;
      (a->intv1)++;
      break;
    case MOBKP_MINKNAP_RIGHT:
      pos = a->intv2;
      (a->intv2)--;
      break;
    default:
...
void improvesolution(allinfo *a, state *v) {
  if (v->wsum > a->c)
    throw("wrong improvesoluton");
  if (v->psum <= a->z)
    throw("not improved solution");

  a->z = v->psum;
  a->zwsum = v->wsum;
  a->ovect = v->vect;
  std::memcpy(a->ovitem, a->vitem, sizeof(item *) * MOBKP_MINKNAP_MAXV);
}
...
void definesolution(allinfo *a) {
  ...
  if (a->firsttime) {
    a->zstar = a->z;
    a->firsttime = MOBKP_MINKNAP_FALSE;
  }
  ...
  for (j = 0; j < MOBKP_MINKNAP_MAXV; j++) {
    k = a->ovect & (static_cast<btype>(1) << j);
    i = a->ovitem[j];
    if (i == NULL)
      continue;
    if (*(i->x) == 1) {
...
item *median(item *f1, item *l1, ntype s) {
  ...
  for (;;) {
    ...
    if (d > 1) {
      if (MOBKP_MINKNAP_DET(f->p, f->w, m->p, m->w) < 0)
        MOBKP_MINKNAP_SWAP(f, m);
      if (d > 2) {
        if (MOBKP_MINKNAP_DET(m->p, m->w, l->p, l->w) < 0) {
          MOBKP_MINKNAP_SWAP(m, l);
          if (MOBKP_MINKNAP_DET(f->p, f->w, m->p, m->w) < 0)
            MOBKP_MINKNAP_SWAP(f, m);
        }
      }
    ...
    for (;;) {
      do {
        i += s;
      } while (MOBKP_MINKNAP_DET(i->p, i->w, mp, mw) > 0);
      do {
        j -= s;
      } while (MOBKP_MINKNAP_DET(j->p, j->w, mp, mw) < 0);
      if (i > j)
        break;
      MOBKP_MINKNAP_SWAP(i, j);
    }
...
void partsort(allinfo *a, item *f, item *l, stype ws, int what) {
  ...
  d = l - f + 1;
  if (d < 1)
    throw("negative interval in partsort");
  if (d > MOBKP_MINKNAP_MINMED) {
    m = median(f, l, static_cast<int>(std::sqrt(d)));
  } else {
    if (d > 1) {
      m = f + d / 2;
      if (MOBKP_MINKNAP_DET(f->p, f->w, m->p, m->w) < 0)
  ...
  if (d > 3) {
    ...
    if (wi <= a->cstar) {
      if (what == MOBKP_MINKNAP_SORTALL)
        partsort(a, f, i - 1, ws, what);
      if (what == MOBKP_MINKNAP_PARTIATE)
        push(a, MOBKP_MINKNAP_LEFT, f, i - 1);
      partsort(a, i, l, wi, what);
    } else {
      if (what == MOBKP_MINKNAP_SORTALL)
        partsort(a, i, l, wi, what);
      if (what == MOBKP_MINKNAP_PARTIATE)
        push(a, MOBKP_MINKNAP_RIGHT, i, l);
      partsort(a, f, i - 1, ws, what);
    }
  }

  if ((d <= 3) || (what == MOBKP_MINKNAP_SORTALL)) {
    a->fpart = f;
    a->lpart = l;
    a->wfpart = ws;
  }
...
boolean haschance(allinfo *a, item *i, int side) {
  ...
  if (a->d.size == 0)
    return MOBKP_MINKNAP_FALSE;

  if (side == MOBKP_MINKNAP_RIGHT) {
    if (a->d.fset->wsum <= a->c - i->w)
      return MOBKP_MINKNAP_TRUE;
    p = a->ps;
    w = a->ws;
    a->pitested++;
    pp = i->p - a->z - 1;
    ww = i->w - a->c;
    r = -MOBKP_MINKNAP_DET(pp, ww, p, w);
    for (j = a->d.fset, m = a->d.lset + 1; j != m; j++) {
      if (MOBKP_MINKNAP_DET(j->psum, j->wsum, p, w) >= r)
        return MOBKP_MINKNAP_TRUE;
    }
  } else {
...
void multiply(allinfo *a, item *h, int side) {
  ...
  if (2 * a->d.size + 2 > MOBKP_MINKNAP_MAXSTATES)
    throw("no space in multiply");

  /* keep track on solution vector */
  a->vno++;
  if (a->vno == MOBKP_MINKNAP_MAXV)
    a->vno = 0;
  mask1 = (static_cast<btype>(1) << a->vno);
  mask0 = ~mask1;
  a->vitem[a->vno] = h;
  ...
  m = rm + 1;
  k->psum = -1;
  k->wsum = r1->wsum + std::abs(p) + 1;
  m->wsum = rm->wsum + std::abs(w) + 1;
  ...
  a->coresize++;
  if (a->d.size > a->maxstates)
    a->maxstates = a->d.size;
}
...
void simpreduce(int side, item **f, item **l, allinfo *a) {
  ...
  pb = a->b->p;
  wb = a->b->w;
  q = MOBKP_MINKNAP_DET(a->z + 1 - a->psumb, a->c - a->wsumb, pb, wb);
  r = -MOBKP_MINKNAP_DET(a->z + 1 - a->psumb, a->c - a->wsumb, pb, wb);
  i = *f;
  j = *l;
  redu = 0;
  if (side == MOBKP_MINKNAP_LEFT) {
    k = a->fsort - 1;
    while (i <= j) {
      if (MOBKP_MINKNAP_DET(j->p, j->w, pb, wb) > r) {
        MOBKP_MINKNAP_SWAP(i, j);
        i++;
        redu++; /* not feasible */
      } else {
        MOBKP_MINKNAP_SWAP(j, k);
        j--;
        k--; /* feasible */
      }
    }
    ...
    *f = k + 1;
    ...
    k = a->lsort + 1;
    while (i <= j) {
      if (MOBKP_MINKNAP_DET(i->p, i->w, pb, wb) < q) {
        MOBKP_MINKNAP_SWAP(i, j);
        j--;
        redu++; /* not feasible */
      } else {
        MOBKP_MINKNAP_SWAP(i, k);
        i++;
        k++; /* feasible */
      }
    }
...
void reduceset(allinfo *a) {
  ...
  if (a->s < a->fsort) {
    if (a->intv1 == a->intv1b) {
      ps = MOBKP_MINKNAP_PMAX;
      ws = MOBKP_MINKNAP_WMAX;
    } else {
      pop(a, MOBKP_MINKNAP_LEFT, &f, &l);
      if (f < a->ftouch)
        a->ftouch = f;
      ps = f->p;
      ws = f->w; /* default: pick first item */
      simpreduce(MOBKP_MINKNAP_LEFT, &f, &l, a);
      if (f <= l) {
        partsort(a, f, l, 0, MOBKP_MINKNAP_SORTALL);
        a->fsort = f;
        ps = a->s->p;
        ws = a->s->w;
      }
  ...
  if (a->t > a->lsort) {
    if (a->intv2 == a->intv2b) {
      pt = MOBKP_MINKNAP_PMIN;
      wt = MOBKP_MINKNAP_WMIN;
    } else {
      pop(a, MOBKP_MINKNAP_RIGHT, &f, &l);
      if (l > a->ltouch)
        a->ltouch = l;
      pt = l->p;
      wt = l->w; /* default: pick first item */
      simpreduce(MOBKP_MINKNAP_RIGHT, &f, &l, a);
      if (f <= l) {
        partsort(a, f, l, 0, MOBKP_MINKNAP_SORTALL);
        a->lsort = l;
        pt = a->t->p;
  ...
  r = MOBKP_MINKNAP_DET(z, c, ps, ws);
  for (i = rm, m = v; i != m; i--) {
    if (MOBKP_MINKNAP_DET(i->psum, i->wsum, ps, ws) >= r) {
      k--;
      *k = *i;
    }
  }

  r = MOBKP_MINKNAP_DET(z, c, pt, wt);
  for (i = v, m = r1 - 1; i != m; i--) {
    if (MOBKP_MINKNAP_DET(i->psum, i->wsum, pt, wt) >= r) {
      k--;
      *k = *i;
    }
  }
...
void initfirst(allinfo *a, stype ps, stype ws) {
  ...
  a->d.size = 1;
  a->d.set1 = new state[MOBKP_MINKNAP_MAXSTATES];
  a->d.setm = a->d.set1 + MOBKP_MINKNAP_MAXSTATES - 1;
  a->d.fset = a->d.set1;
  a->d.lset = a->d.set1;
...
void initvect(allinfo *a) {
  btype i;
  for (i = 0; i < MOBKP_MINKNAP_MAXV; i++)
    a->vitem[i] = NULL;
  a->vno = MOBKP_MINKNAP_MAXV - 1;
}
...
void copyproblem(item *f, item *l, itype *p, itype *w, int *x) {
  item *i, *m;
  itype *pp, *ww;
  int *xx;
...
void findbreak(allinfo *a) {
  ...
  a->psumb = psum;
  a->wsumb = wsum;
  a->dantzig = psum + ((c - wsum) * static_cast<ptype>(i->p)) / i->w;
...
stype minknap(int n, itype *p, itype *w, int *x, itype c) {
  allinfo a;
  item *tab;
  interval *inttab;
  ...
  inttab = new interval[MOBKP_MINKNAP_SORTSTACK];
  a.intv1 = a.intv1b = &inttab[0];
  a.intv2 = a.intv2b = &inttab[MOBKP_MINKNAP_SORTSTACK - 1];
  a.fsort = a.litem;
  a.lsort = a.fitem;
  partsort(&a, a.fitem, a.litem, 0, MOBKP_MINKNAP_PARTIATE);
  findbreak(&a);

  a.ub = a.dantzig;
  a.firsttime = MOBKP_MINKNAP_TRUE;
  ...
  for (;;) {
    ...
    while ((a.d.size > 0) && (a.z < a.ub)) {
      if (a.t <= a.lsort) {
        if (haschance(&a, a.t, MOBKP_MINKNAP_RIGHT))
          multiply(&a, a.t, MOBKP_MINKNAP_RIGHT);
        (a.t)++;
      }
      reduceset(&a);
      if (a.s >= a.fsort) {
        if (haschance(&a, a.s, MOBKP_MINKNAP_LEFT))
          multiply(&a, a.s, MOBKP_MINKNAP_LEFT);
        (a.s)--;
      }
      reduceset(&a);
    }
...
#undef MOBKP_MINKNAP_DET
#undef MOBKP_MINKNAP_SWAP
#undef MOBKP_MINKNAP_DIFF
#undef MOBKP_MINKNAP_NO
#undef MOBKP_MINKNAP_N
#undef MOBKP_MINKNAP_L
#undef MOBKP_MINKNAP_SZ
